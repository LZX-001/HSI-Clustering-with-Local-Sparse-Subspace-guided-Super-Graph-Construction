...
function ssc_coef = SSC_HSI(X,Con, r, affine, alpha, outlier, rho)
...
if (~outlier)
    CMat = admmLasso_mat_func(Xp,Con, affine, alpha);
    C = CMat;
else
    CMat = admmOutlier_mat_func(Xp,Con, affine, alpha);
    N = size(Xp, 2);
    C = CMat(1:N, :);
end

ssc_coef = BuildAdjacency(thrC(C, rho));
% ssc_coef=ssc_coef.*Con;
